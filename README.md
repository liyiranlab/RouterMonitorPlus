...
|RouterMonitor+ |
|----|
| ![routermonitor+](./images/) |

|RouterMonitor+Power |
|  ----  |
| ![RouterMonitor+Power](./images/) |

上图就是我改进后的作品，改进的过程中发现很多问题，下面会说明，把SD2小电视的1117LDO改DCDC供电后，优化代码，平均功耗做到135mW左右，即使SD2小电视硬件没有任何改动，用我提供的开源代码也可降低现有功耗的50%。  
...
8 关于ESP8266连接WIFI时，降低功耗要注意的问题，发现ESP8266连接带WIFI的主路由，或二级路由的时候，8266的功耗最低，但是也建议开启2.4gWIFI客户端隔离，信标间隔 500，等隔离功能，目的是降低8266和wifi的通讯数据，比如广播等等没有必要的通信，二图是小电视连接AX6开源固件二级路由器模式时候测试的功率；  
9 关于ESP8266连接桥接AP时候，功耗较高于连接主路由情况，解决办法，开启2.4gwifi的，WIFI客户端隔离，组播转单播，DTIM 间隔设置为5，信标间隔 500，尽可能降低8266和桥接AP的通信数据，但是实际功率受到同网段中设备多少的影响，可以固化IP地址防止ARP，防火墙设置禁止IPV6相关与8266通信  
...
13 改DCDC供电，建议输入输出增加大电容，出入大电容前先分别串入1206封装600R磁珠；  

|不带CH430下载芯片时的深睡眠功耗 |
|  ----  |
| ![不带CH430下载芯片时的深睡眠功耗](./images/NoCH340DP.png) |

|带CH430下载芯片断开芯片GND时深睡眠功耗 |
|  ----  |
| ![带CH430下载芯片断开芯片GND时深睡眠功耗](./images/YesVH340DPoff.png) |

|带CH430下载芯片不断开芯片GND连接5V电源深睡眠功耗 |
|  ----  |
| ![带CH430下载芯片不断开芯片GND连接5V电源深睡眠功耗](./images/YesVH340DPNoPower.png) |

|带CH430下载芯片不断开芯片GND连接PC上位机时深睡眠功耗 |
|  ----  |
| ![带CH430下载芯片不断开芯片GND连接PC上位机时深睡眠功耗](./images/YesVH340DPNoPC.png) |
