...
// 监控数据更新频率(秒)
const unsigned long BATCH_REQUEST_INTERVAL = 1000; // 1秒监控数据更新时间
// ===== 控制全量数据请求的间隔次数 =====
...
const unsigned long LOGGED_IN_REFRESH_INTERVAL = 499; // 登录后刷新间隔(ms)待机时间
...
const unsigned long DATA_REQUEST_INTERVAL = 499;  // 毫秒间隔发起一次新请求
...
void dynamicAdjustTxPower(int currentRSSI) {
    #if defined(DEBUG_ENABLED_WIFI) || defined(DEBUG_ENABLED_0)
    Serial.printf("[WiFi-PWR] TTL=%lu | Function called with RSSI=%d\n", millis(), currentRSSI);
    #endif
    if (!ENABLE_DYNAMIC_TX_POWER) return;
    
    static float currentPower = WIFI_TX_POWER_DBM;
    static int hysteresisCounter = 0;  // 【新增】滞后计数器，防止瞬时波动
    const int HYSTERESIS_THRESHOLD = 2; // 【新增】连续2次确认才切换功率
    ...
    // 【修复】增加滞后带：使用不同阈值上下限，避免在-60和-70附近振荡
    // 原逻辑：> -60降功率，< -70提功率，中间用默认
    // 新逻辑：增加缓冲区间，信号必须持续改善/恶化才切换
    
    if (currentRSSI > RSSI_GOOD_THRESHOLD + 5) {        // > -55 dBm（严格的好信号）
        newPower = 12.0;
        #if defined(DEBUG_ENABLED_WIFI) || defined(DEBUG_ENABLED_0)
        reason = "Signal excellent (>-55dBm), reduce power";
        #endif
    } 
    else if (currentRSSI < RSSI_MEDIUM_THRESHOLD - 5) { // < -75 dBm（严格的差信号）
        newPower = 20.5;  // 【修改】统一最大值为20.5，与ESP8266最大值一致
        #if defined(DEBUG_ENABLED_WIFI) || defined(DEBUG_ENABLED_0)
        reason = "Signal poor (<-75dBm), max power";
        #endif
    }
    else if (currentRSSI > RSSI_GOOD_THRESHOLD) {       // -55 ~ -60 dBm
        // 好信号区域，但留缓冲
        if (currentPower > 12.0) {
            newPower = 12.0;
            #if defined(DEBUG_ENABLED_WIFI) || defined(DEBUG_ENABLED_0)
            reason = "Signal good (>-60dBm), reduce power";
            #endif
        }
    }
    else if (currentRSSI < RSSI_MEDIUM_THRESHOLD) {       // -70 ~ -75 dBm
        // 弱信号区域，但留缓冲
        if (currentPower < 18.5) {
            newPower = 18.5;
            #if defined(DEBUG_ENABLED_WIFI) || defined(DEBUG_ENABLED_0)
            reason = "Signal weak (<-70dBm), increase power";
            #endif
        }
    }
    else {
        // -60 ~ -70 dBm 中间区域：保持当前功率不变（【新增】滞后带）
        newPower = currentPower;
        #if defined(DEBUG_ENABLED_WIFI) || defined(DEBUG_ENABLED_0)
        reason = "Signal medium (-70~-60dBm), keep current";
        #endif
    }
    
    // 【新增】滞后处理：只有功率目标真正改变，且连续确认才执行
    if (newPower != currentPower) {
        hysteresisCounter++;
        if (hysteresisCounter >= HYSTERESIS_THRESHOLD) {
            // 确认切换
            WIFI_POWER_LOG(currentPower, newPower, currentRSSI, reason);
            setWiFiTxPower(newPower);
            currentPower = newPower;
            hysteresisCounter = 0;  // 重置计数器
        }
#if defined(DEBUG_ENABLED_WIFI) || defined(DEBUG_ENABLED_0)
        else {
            Serial.printf("[WiFi-PWR] TTL=%lu | RSSI=%d | Pending change %.1f->%.1f (%d/%d)\n",
                         millis(), currentRSSI, currentPower, newPower, 
                         hysteresisCounter, HYSTERESIS_THRESHOLD);
        }
#endif
    } else {
        // 功率目标未变，重置滞后计数器
...
void handleWiFiConnection()
{
    ...
    case WIFI_STATE_CONNECTING:
        ...
        case WIFI_STATE_CONNECTED:{
            ...
            const unsigned long SLOW_CHECK_INTERVAL = 180000; // 稳定阶段：3分钟
            const unsigned long FAST_PHASE_DURATION = 300000; // 前5分钟为快速阶段
...
void loop()
{
    ...
    auto isWiFiBusy = []() -> bool {
        return (httpCtx.state != HTTP_IDLE && 
    ...
    // 仅在 WiFi 空闲 +（有新数据 或 保底时间到）时刷新
        if (!isWiFiBusy() && (hasNewData || (millis() - lastRefreshTime >= LOGGED_IN_REFRESH_INTERVAL))){
